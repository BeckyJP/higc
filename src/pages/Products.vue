<template>
  <DefaultLayout>
    <div class="container mx-auto md:px-20 px-8 md:my-10 my-8">
      <div class="grid grid-cols-1 md:grid-cols-2 py-4 mt-6">
        <div>
          <small class="font-mono uppercase text-gray-500">Main Services</small>
          <h1 class="font-neue md:text-[56px] my-3 text-[30px] md:leading-[4rem]">
            Essential Medical Technology for
            <span class="text-[#ABABAB]">Superior Healthcare</span>
          </h1>
        </div>
        <div class="pt-12 md:px-8">
          <p class="text-gray-500 leading-[1.5rem] text-[16px]">
            Transform your healthcare facility with our comprehensive range of medical
            equipment and devices, sourced from trusted manufacturers and backed by CE
            certifications where applicable.
          </p>
          <div class="flex flex-wrap gap-4 mt-10">
            <BaseButton> Contact Us to Learn More </BaseButton>
            <BaseButton variant="secondary"> Partnership Opportunities </BaseButton>
          </div>
        </div>
      </div>
    </div>
    <section class="bg-darkGray md:py-10 my-10">
      <div class="container mx-auto md:px-20 px-8">
        <div class="md:px-8 px-2 mb-10">
          <small class="text-gray-500 font-mono uppercase text-sm my-3"
            >Stay Equipped</small
          >
          <h1
            class="font-neue text-black md:text-[40px] py-3 mt-2 text-2xl md:leading-[2rem]"
          >
            Medical Equipments & Devices
          </h1>
          <p class="text-gray-500 text-base">
            Discover reliable devices and tools designed to support accurate care and
            efficient practice.
          </p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <img
              src="@/assets/HIGCImages/products/Product1.png"
              alt="HIGC image"
              class="object-container md:object-cover rounded-t-2xl"
            />
            <div class="px-6 py-4">
              <p class="text-[18px] text-black font-neue mt-2">BLS and ACLS Ambulances</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product2.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                On-site Testing Machines
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product3.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">BP monitors</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product4.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Stethoscopes</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product5.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Syringes, Needles, and Cannulas
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product6.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Rapid Test Kits</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product7.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Drug and Alcohol Test Kits
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product8.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Hospital and diagnostic equipment
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product9.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                CPAP, BIPAP Machines and Ventilators
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product10.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Autoclaves</p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="bg-white md:py-10 my-10">
      <div class="container mx-auto md:px-20 px-8">
        <div class="md:px-8 px-2 mb-10">
          <small class="text-gray-500 font-mono uppercase text-sm my-3"
            >Stay Stocked</small
          >
          <h1
            class="font-neue text-black md:text-[40px] py-3 mt-2 text-2xl md:leading-[2rem]"
          >
            Medical Supplies & Consumables
          </h1>
          <p class="text-gray-500 text-base">
            Access essential supplies and consumables that keep your facility prepared for
            daily demands.
          </p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <img
              src="@/assets/HIGCImages/products/Product11.png"
              alt="HIGC image"
              class="object-container md:object-cover rounded-t-2xl"
            />
            <div class="px-6 py-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Calibrated Maternity Drapes
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product12.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Calibrated Maternity Trays
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product13.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Physician Starter Kits</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product14.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">First Aid Kits</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product15.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Nitrile Powder-Free Examination Gloves
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product16.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Latex Powdered Examination Gloves
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product17.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Sterile Surgical Gloves</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product18.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Isolation and Surgical Gowns
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product19.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Biohazard Spill Kits</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product20.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Biohazard Bags</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product21.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Biohazard Sharps Container
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product22.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Disinfectant Products</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Product23.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Thermometers</p>
            </div>
          </div>
          <div class="">
            <img
              src="@/assets/HIGCImages/products/Product24.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Face masks</p>
            </div>
          </div>
          <div class="">
            <img
              src="@/assets/HIGCImages/products/Products23.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Face shields</p>
            </div>
          </div>
          <div class="">
            <img
              src="@/assets/HIGCImages/products/Product25.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Reagents and Lab Cleaning Solutions
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section class="bg-darkGray md:py-10">
      <div class="container mx-auto md:px-20 px-8">
        <div class="md:px-8 px-2 mb-10">
          <small class="text-gray-500 font-mono uppercase text-sm my-3"
            >Stay Protected</small
          >
          <h1
            class="font-neue text-black md:text-[40px] py-3 mt-2 text-2xl md:leading-[2rem]"
          >
            Safety & Protection Equipment
          </h1>
          <p class="text-gray-500 text-base">
            Equip your staff with trusted gear to ensure safety in every healthcare
            environment.
          </p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <img
              src="@/assets/HIGCImages/products/safety1.png"
              alt="HIGC image"
              class="object-container md:object-cover rounded-t-2xl"
            />
            <div class="px-6 py-4">
              <p class="text-[18px] text-black font-neue mt-2">Biohazard Coveralls</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety2.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Protective eye goggles</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety3.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Impact-resistant and customized hard hats
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety4.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Fire retardant coveralls
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety5.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Safety shoes</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety6.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Reflective jackets</p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/safety7.png"
              alt="HIGC image"
              class="h-auto object-container md:object-cover rounded-t-2xl"
            />
            <div class="p-4">
              <p class="text-[18px] text-black font-neue mt-2">Life jackets</p>
            </div>
          </div>
        </div>
      </div>
    </section>
    <div class="bg-white md:py-10">
      <div class="container mx-auto md:px-20 px-8">
        <div class="md:px-8 px-2 mb-10">
          <small class="text-gray-500 font-mono uppercase text-sm my-3">Stay Ready</small>
          <h1
            class="font-neue text-black md:text-[40px] py-3 mt-2 text-2xl md:leading-[2rem]"
          >
            Training Equipment
          </h1>
          <p class="text-gray-500 text-base">
            Build confidence and skills with training tools and resources made for
            real-world situations.
          </p>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <img
              src="@/assets/HIGCImages/products/Equ1.png"
              alt="HIGC image"
              class="object-container md:object-cover rounded-t-2xl"
            />
            <div class="px-6 py-4">
              <p class="text-[18px] text-black font-neue mt-2">
                Mannequins and HSSE training tools
              </p>
            </div>
          </div>
          <div>
            <img
              src="@/assets/HIGCImages/products/Equ2.png"
              alt="HIGC image"
              class="object-container md:object-cover rounded-t-2xl"
            />
            <div class="px-6 py-4">
              <p class="text-[18px] text-black font-neue mt-2">Hospital furniture</p>
            </div>
          </div>
        </div>
      </div>
    </div>
    <section class="bg-darkGray md:py-10">
      <div class="container mx-auto md:px-20 px-8">
        <div class="md:px-8 px-2 mb-10">
          <small class="text-gray-500 font-mono uppercase text-sm my-3"
            >PLACES WE HAVE BEEN RECOGNISED
          </small>
          <h1
            class="font-neue text-black md:text-[56px] py-3 mt-4 text-2xl md:leading-[2rem]"
          >
            Universal Product Features
          </h1>
          <p class="text-gray-500 text-base">
            5 A creative, curious crew building things we believe in—together.
          </p>
          <div>
            <h2
              class="font-neue text-black md:text-[40px] py-6 mt-8 text-2xl md:leading-[2rem]"
            >
              Quality Certifications
            </h2>
            <ul class="font-neue">
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>CE Marking: </b>European Conformity for applicable medical devices
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>ISO Standards:</b> Quality management system compliance
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>FDA Approval: </b>US regulatory approval for relevant products
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>Nigerian Standards:</b> Local regulatory compliance certification
              </li>
            </ul>
            <h2
              class="font-neue text-black md:text-[40px] py-6 mt-8 text-2xl md:leading-[2rem]"
            >
              Ordering Process
            </h2>
            <ul class="font-neue">
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>Consultation:</b> Product selection guidance from our technical team
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>Purchase Order: </b>Formal order processing and confirmation
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b> Delivery:</b> Nigeria-wide distribution with tracking
              </li>
              <li class="bg-gray-100 py-4 my-4 rounded-lg px-4 text-base">
                <b>Support: </b> Installation, training, and after-sales service
              </li>
            </ul>
          </div>
        </div>
      </div>
    </section>

    <div class="bg-white mt-10">
      <ContactForm />
    </div>
  </DefaultLayout>
</template>

<script setup>
import BaseButton from "@/components/base/BaseButton.vue";
import DefaultLayout from "@/layouts/DefaultLayout.vue";
import ContactForm from "@/components/ContactForm.vue";
</script>
